...
class MetadataAuthenticationProvider(AuthenticationProvider):
    """
    ...
    def __init__(self, config: MetadataConnection):
        self.config = config
        self.security_config: MetadataJWTClientConfig = self.config.securityConfig
        self.jwt_token = None
        self.expiry = datetime.now() - relativedelta(years=1)

    @classmethod
    def create(cls, config: MetadataConnection):
        return cls(config)
