...
class MinIOProfilerSource(ProfilerSourceInterface):
    ...
    def __init__(
        ...
        self.service_conn_config = self._copy_service_config(config, storage)
    ...
    @staticmethod
    def get_config_for_container(entity: Container, profiler_config) -> Optional[Container]:
    ...
    def _copy_service_config(self, config: OpenMetadataWorkflowConfig, storage: StorageService
    ) -> StorageConnection:
    ...
    def get_profiler_runner(
        ...
        """
        table_config = self.get_config_for_container(entity, profiler_config)
