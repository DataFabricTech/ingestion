...
class TestGreatExpectationIntegration(TestCase):
    ...
    server_config = MetadataConnection(
        hostPort=WORKFLOW_CONFIG["serverConfig"]["hostPort"],
        authProvider=WORKFLOW_CONFIG["serverConfig"]["authProvider"],
        securityConfig=MetadataJWTClientConfig(
            jwtToken=WORKFLOW_CONFIG["serverConfig"]["securityConfig"][
                "jwtToken"
            ]
        ),
    )  # type: ignore
