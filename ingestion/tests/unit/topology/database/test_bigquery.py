...
class BigqueryUnitTest(TestCase):
    ...
    def __init__(
        ...
        self.config = MetadataWorkflowConfig.parse_obj(mock_bq_config)
        self.metadata = ServerInterface(
            MetadataConnection.parse_obj(
                mock_bq_config["workflowConfig"]["serverConfig"]
            )
        )
...
class BigqueryLineageSourceTest(TestCase):
    ...
    def __init__(
        ...
        self.config = MetadataWorkflowConfig.parse_obj(
            mock_credentials_path_bq_config
        )
        self.bq_query_parser = BigqueryLineageSource(
            self.config.source, self.config.workflowConfig.serverConfig
        )
